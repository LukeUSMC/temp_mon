...
int PietteTech_DHT::acquire() {
    ...
    if (_state == STOPPED || _state == ACQUIRED) {
         ...
         */
        Serial1.println("Toggling pinMode.");
        pinMode(_sigPin, OUTPUT);
        Serial1.println("digitalWrite Low.");
        digitalWrite(_sigPin, LOW);
        Serial1.println("Went Low.");
        if (_type == DHT11){
        Serial1.println("Starting Delay 20ms.");
          HAL_Delay_Milliseconds(20);                  // DHT11 Spec: 18ms min
          }
        else{
        Serial1.println("Starting Microseconds Delay.");
        HAL_Delay_Microseconds(1500);    // DHT22 Spec: 0.8-20ms, 1ms typ
      };
        Serial1.println("Try pinMode Toggle to Input.");
        pinMode(_sigPin, INPUT);        // Note Hi-Z mode with pullup resistor
                                        // will keep this high until the DHT responds.
        Serial1.println("Back to Input.");
        /*
         * Attach the interrupt handler to receive the data once the DHT
         * starts to send us data
         */
        _us = HAL_Timer_Get_Micro_Seconds();
        Serial1.print("Current Micros: ");
        Serial1.println(_us);
        Serial1.println("Attaching Interrupt.");
        attachInterrupt(_sigPin, isrCallback_wrapper, FALLING);

        return DHTLIB_ACQUIRING;
    } else
        Serial1.println("Returning Error.");
